...
/* Solar PV Design Platform v2.1 - corrected PV layout engine.
   Ground-mount row axes are constrained perpendicular to the configured surface azimuth
   when tilt is non-zero so Grid-U pitch and the no-shadow calculation share one physical axis. */
...
function getPanelGridDimensions(moduleWidthM, moduleHeightM, orientation) {
  const moduleWidth = Number(moduleWidthM);
  const moduleHeight = Number(moduleHeightM);
  if (!(moduleWidth > 0) || !(moduleHeight > 0)) return { width: 0, length: 0 };
  return orientation === "Landscape"
    ? { width: moduleHeight, length: moduleWidth }
    : { width: moduleWidth, length: moduleHeight };
}

function calculateGroundNoShadowPitch(chordLengthM, tiltDeg, solarElevationDeg) {
  const chord = Number(chordLengthM);
  const tilt = Number(tiltDeg);
  const elevation = Number(solarElevationDeg);
  if (!(chord > 0) || !Number.isFinite(tilt) || tilt < 0 || tilt >= 90 || !Number.isFinite(elevation) || elevation <= 0 || elevation >= 90) return null;
  const tiltRad = tilt * Math.PI / 180;
  const elevationRad = elevation * Math.PI / 180;
  return chord * Math.cos(tiltRad) + chord * Math.sin(tiltRad) / Math.tan(elevationRad);
}

function getAzimuthCandidates(face, origin, settings) {
  // For a tilted ground-mount table, Grid-U is the row-to-row pitch direction.
  // Align Grid-U with the configured surface azimuth, so the physical row axis
  // (Grid-V) is perpendicular to tilt direction. This keeps the no-shadow pitch
  // calculation and the packing grid geometrically consistent.
  if (getInstallationType() === "Ground-Mounted Solar" && Math.abs(Number(face?.roofTilt) || 0) > 1e-9) {
    return [normalizeLayoutAxisAzimuth((Number(face?.roofAzimuth) || 0) - 90)];
  }
...
function buildCandidateLayoutForFace(face, settings) {
  ...
  for (const orientation of orientationCandidates) {
    const panelDimensions = getPanelGridDimensions(settings.moduleWidth, settings.moduleHeight, orientation);
    const panelWidth = panelDimensions.width;
    const panelLength = panelDimensions.length;
  ...
  const chosen = best || {
    ...
    panelWidth: getPanelGridDimensions(settings.moduleWidth, settings.moduleHeight, settings.orientation).width,
    panelLength: getPanelGridDimensions(settings.moduleWidth, settings.moduleHeight, settings.orientation).length,
    cells: [], panelCount: 0,
...
globalThis.SolarPVLayoutEngineModule = Object.freeze({version:"2.1.0",deterministic:true,functions:Object.freeze(["getLayoutOrigin", "latLngToLocalMeters", "localMetersToLatLng", "createLayoutBasis", "localToGrid", "gridToLocal", "createPanelRectangle", "panelFitsSite", "normalizeLayoutAxisAzimuth", "layoutAxisDifference", "layoutAzimuthFromVector", "uniqueLayoutAzimuths", "getMajorRoofEdgeAzimuths", "getGridPhaseFractions", "getOrientationCandidates", "getPanelGridDimensions", "calculateGroundNoShadowPitch", "getAzimuthCandidates", "countGridRuns", "calculateCandidatePracticality", "isBetterLayoutCandidate", "evaluateLayoutPhase", "buildCandidateLayoutForFace", "buildCandidateLayout"])});
