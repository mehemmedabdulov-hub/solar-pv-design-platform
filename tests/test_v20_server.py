...
SERVER_PATH = ROOT / "solar_pv_design_platform_v2.0_server.py"
spec = importlib.util.spec_from_file_location("solar_v20_server", SERVER_PATH)
...
def make_snapshot(revision: int, parent: str | None, name: str) -> dict:
    snapshot = {
        "schemaVersion": "2.0",
        "applicationVersion": "2.0-alpha1",
        "projectId": "v20-server-test",
...
assert server_mod.APP_VERSION == "2.0.0-alpha.1"
...
with tempfile.TemporaryDirectory(prefix="solar-v20-server-") as tmp:
    ...
    try:
        ...
        assert health["version"] == "2.0.0-alpha.1"
        ...
        status, result = request_json(
            base + "/api/solar-pv/projects/v20-server-test/revisions",
            method="POST",
            body=first,
        ...
        # Exact immutable repeat remains idempotent.
        status, result = request_json(
            base + "/api/solar-pv/projects/v20-server-test/revisions",
            method="POST",
            body=first,
        ...
        status, current = request_json(base + "/api/solar-pv/projects/v20-server-test/current")
        assert status == 200
        assert current["snapshot"]["revisionNumber"] == 2
        assert current["snapshot"]["stateFingerprint"] == second["stateFingerprint"]

        status, history = request_json(base + "/api/solar-pv/projects/v20-server-test/revisions")
        assert status == 200
        assert [item["revisionNumber"] for item in history["revisions"]] == [2, 1]

        status, restored = request_json(base + "/api/solar-pv/projects/v20-server-test/revisions/1")
        assert status == 200
        assert restored["snapshot"]["stateFingerprint"] == first["stateFingerprint"]

        # Malformed/mismatched snapshot fingerprint is still rejected by server boundary.
